...
int precedencia (char c){ // Usada no Algoritmo 2
	if( c == '*' || c == '/'){
		return(2);
	}
	else if( c == '+' || c == '-') {  // precedencia menor
		return(1);
	}
	else{
		return(0);
...
void transforma (char* exp_infixa, char* exp_posfixa) { 
	...
	while(item != '\0') {  // percorre toda expressao infixa
		...
		if(item == '(' || item == '[' || item == '{'){
		...
		else if(tem_op (item)){ // caractere eh operador
			
			exp_posfixa[j++] = ' ';      // coloca espaços entre os operandos e operadores
			c = desempilha_c (pilha_c);
			
			while(tem_op (c) && precedencia (c) >= precedencia (item)){  // enquanto c for um operador e sua precedencia for maior que a do item
			...
			empilha_c(c, pilha_c);                      //  desempilhamos 1 operador a mais onde a condição falha e o loop termina, entao para esse caso                     
			empilha_c(item, pilha_c);                  /* empilha operador atual para a pilha */
		}
		else if(item == ')'){                   /* se o simbolo atual eh ')' entao */
			c = desempilha_c (pilha_c);             /* desempilha e continua ateh */
			
			while(c != '('){                   /* '(' for encontrado */
		...
		else{ /* Caso nao seja nenhum dos casos: operando, operador ou separador */
