...
float resolve_posfixa(char *str)
{
	...
	for (i = 0; str[i] != '\0'; i++)
	{ // percorre toda expressao posfixa
		...
		if (tem_numero(str[i]))
		{
			...
			while (tem_numero(str[i]))
			{ // Coleta os operandos e copia na na string num
				num[j++] = str[i++];
			}
		...
		if (tem_op(str[i]))
		{
			...
			{																					 // verifica se pilha possui operandos suficientes (2 valores float)
