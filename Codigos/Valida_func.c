...
for(i=0; str[i] != '\0'; i++){ 
	...
	if(!valida_str(str[i])) { // Verifica se ha char irregular em toda string
		entrada = false;
		break;
	}
   
	if(tem_char(str[i])){ // Verifica se string possui caracteres diferentes de numeros
		cont += 1;
		
		if(str[i] == '!'){ // Caso caractere especial
...
if(cont>1) entrada = false; // Se contador de char > 1, � invalido


if(cont == 1){ // Caso de apenas 1 operador 
...
int num = false;
	...
	if(num){
		...
		for(i=0; str[i] != '\0' ;i++){
			if(tem_operador (str[i])){	 // Verifica se tem (+ - * / ! c )
