...
// Desempilha 2 ultimos numeros float da pilha e faz operação dada, sendo o antepenultimo valor o primeiro operando nos casos ( - e / )
float desempilha_e_calcula_1 (char op, t_pilha* pilha){ // Usada no modo (2) - Calculadora
	...
	if (pilha->primeiro == pilha->ultimo){ //unico elemento
		printf("Operação inválida -> unico elemento\n");
		return false;
	}else{
	...
	float x;    //Recebe resultado da operação
	switch(op){
		case '+':
			x = x1+x2;
			break;
...
// Desempilha 2 ultimos numeros float da pilha e faz operação dada, sendo o antepenultimo valor o segundo operando nos casos ( - e / )
float desempilha_e_calcula_2 (char op,t_pilha* pilha){ // Usada no Modo (1) - Resolucao de Expressao - no Algoritmo_3.c
